...
if(isset($_POST['submit'])) {
    ...
    $checkqsn = "SELECT * FROM sales_journal where in_num='$invoice'";
	$runcheck = mysqli_query($conn , $checkqsn) or die(mysqli_error($conn));
	$qno = mysqli_num_rows($runcheck);

	if($qno==0){

	$query = "INSERT INTO sales_journal VALUES('$invoice','$date','$supplier','$description','$qty','$price','$val','$discount','$tot')" ;
...
<body>
  <nav class="navbar navbar-dark  navbar-expand-lg bg-dark">
    ...
    <section style="margin-top: 2rem;margin-bottom: 80px;">
        <h1>Input data of the the Sales Voucher</h1>
